...
function tf(text: string, answer: boolean): QuestionSpec {
  return {
    type: 'single',
    text,
    points: 1,
...
function open(text: string, points = 1): QuestionSpec {
...
export function seedChidonQuiz() {
  ...
  if (!admin) {
    console.log('No admin account found yet; skipping Chidon quiz seed.');
    return;
  }
  ...
  const seed = db.transaction(() => {
    ...
    QUESTIONS.forEach((q, qi) => {
      ...
      q.choices.forEach((c, ci) => insertChoice.run(questionId, c.text, c.isCorrect ? 1 : 0, ci));
    });
